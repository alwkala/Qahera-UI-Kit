 ...
 *
 * Renders an authoritative semantic SVG icon from icons/registry.yaml (47 icons).
 * Enforces QAHERA-VISUAL-001 (Zero emoji).
 *
 * @var League\Plates\Template\Template $this
 * @var string $name Name of the registered icon
 * @var string|int $size Size token (xs, sm, md, lg, xl) or pixel integer
 * @var string $extraClass Additional CSS classes
 */
$name = $name ?? 'info';
$size = $size ?? 'md';
...
$pixelSize = is_numeric($size) ? (int)$size : ($sizeMap[$size] ?? 20);

// Authoritative SVG paths from icons/registry.yaml (47 icons)
$iconPaths = [
    // Navigation
...
$path = $iconPaths[$name] ?? null;
if (!$path) {
    return '';
}

$isSpin = $name === 'spinner' ? 'qhr-icon--spin' : '';
$classes = trim("qhr-icon qhr-icon--{$name} {$isSpin} {$extraClass}");
?>
<svg width="<?= $pixelSize ?>" height="<?= $pixelSize ?>" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="<?= htmlspecialchars($classes, ENT_QUOTES, 'UTF-8') ?>" aria-hidden="true">
    <path d="<?= $path ?>" />
</svg>
