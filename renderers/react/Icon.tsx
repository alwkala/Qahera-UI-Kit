...
const SIZE_MAP: Record<QaheraSize, number> = {
  xs: 12,
  sm: 16,
  md: 20,
  lg: 24,
...
// Canonical SVG paths from icons/registry.yaml (41 semantic icons)
...
export const Icon: React.FC<IconProps> = ({
  ...
  if (!pathD) {
    console.warn(`[Qahera Icon] Unknown icon name "${name}". Registered icons: 41.`);
    return null;
  }
  ...
  return (
    <svg
      ...
      className={`qhr-icon qhr-icon--${name} ${isSpinner ? 'qhr-icon--spin' : ''} ${className}`.trim()}
      aria-hidden="true"
      {...props}
    >
      <path d={pathD} />
